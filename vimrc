...
if WINDOWS()
  ...
  let $MYCACHEDIR = $HOME . '/.cache/vim'
endif

if WINDOWS()
  let $MYPLUGGEDDIR = $MYVIMRCDIR . '\plugged'
else
  let $MYPLUGGEDDIR = $HOME . '/.vim/plugged'
endif
...
" Vim UI {
  ...
  " Highlight task tags
  highlight Important ctermfg=Yellow cterm=underline,bold guifg=#CCCC00 gui=underline,bold
  autocmd WinEnter,Syntax * call matchadd('Important', '\W\zs\(IMPORTANT\|HACK\)')

  highlight Note ctermfg=Green cterm=underline,bold guifg=#00DD00 gui=underline,bold
...
" Buildings {
  ...
  function! RepeatBuild()
    if exists('g:project_root_make_last_cmd')
      exec 'AsyncRun -cwd=<root> ' . g:project_root_make_last_cmd
    endif
  endfunction
